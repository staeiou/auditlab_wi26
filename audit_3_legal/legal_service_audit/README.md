# Experiment: Legal Services Recommendation Audit 

This repository contains the full workflow for Experiemnt III, a controlled perturbation audit examing whether large language models (LLMs) assign different recommendation levels to people seeking legal advice in a housing dispute.

The LLM is provided an identical prompt for every call, and the only difference is the person's name, disability status, and lease type. Gender and ethnicity markers are embedded in each name, and a neutral control variable was also included.
...
├─ results.csv              # Dataset obtained and cleaned from legal services audit
